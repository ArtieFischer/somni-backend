-- Save the interpretation with the correct column name

-- 1. Check what's already in the interpreters table
SELECT id, name, full_name FROM interpreters;

-- 2. Check the exact column name in interpretations table
SELECT column_name 
FROM information_schema.columns 
WHERE table_name = 'interpretations' 
  AND column_name LIKE '%interpreter%';

-- 3. Check if the interpretation already exists (using correct column name)
SELECT 
    i.*,
...
-- 4. Save the interpretation
INSERT INTO interpretations (
    dream_id,
    interpreter_id,  -- This is the correct column name based on the table structure
    interpretation,
    advice,
    key_symbols,
    mood_analysis,
...
-- 6. Verify the interpretation was saved by checking again
SELECT 
    i.id,
    i.dream_id,
    i.interpreter_id,
