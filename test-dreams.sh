...
# Enhanced Jungian Analysis with Casual, Absurd Dreams
# Run these to test the "wow" factor of our new interpretation system
...
# Test 1: Classic Absurd Family Dream
echo ""
echo "🏠 Test 1: Family House with Animals & Hair Loss"
echo "------------------------------------------------"
...
# Test 2: Workplace Chaos Dream
echo ""
echo "💼 Test 2: Office Turned Into Jungle Gym"
echo "----------------------------------------"
curl -X POST "$BASE_URL" \
  -H "Content-Type: application/json" \
  -H "X-API-Secret: $API_SECRET" \
...
# Test 3: School Anxiety Classic
echo ""
echo "📚 Test 3: Unprepared for Weird Exam"
echo "------------------------------------"
curl -X POST "$BASE_URL" \
  -H "Content-Type: application/json" \
  -H "X-API-Secret: $API_SECRET" \
...
# Test 4: Transportation Nightmare
echo ""
echo "🚗 Test 4: Car That Keeps Changing"
echo "----------------------------------"
curl -X POST "$BASE_URL" \
  -H "Content-Type: application/json" \
  -H "X-API-Secret: $API_SECRET" \
...
# Test 5: Celebrity Encounter Absurdity  
echo ""
echo "⭐ Test 5: Celebrity Cooking Disaster"
echo "------------------------------------"
curl -X POST "$BASE_URL" \
  -H "Content-Type: application/json" \
  -H "X-API-Secret: $API_SECRET" \
...
echo "   • Authentic Jung voice ('I notice...', 'What strikes me...')"
echo "   • Age-specific insights and life phase awareness"
echo "   • Core tension identification in absurd scenarios"
echo "   • Personal and meaningful symbol interpretation"
echo "   • Profound questions that create lasting impact"
echo ""
echo "🔥 The absurd becomes profound through Jungian lens!" 