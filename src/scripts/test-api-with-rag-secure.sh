...
if [ -z "$API_SECRET_KEY" ]; then
  echo "❌ API_SECRET_KEY not found in .env!"
  exit 1
fi
...
echo ""
echo "📝 Note: This uses the TEST endpoint (/test/interpret) which only requires"
echo "   API secret, not user authentication. Perfect for testing RAG!"